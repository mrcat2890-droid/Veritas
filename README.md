...
**VERITAS** (*Channel Switch Announcement Attack & Audit Framework*) adalah alat evaluasi dan audit keamanan jaringan nirkabel native C11 berkinerja ultra-tinggi. Dirancang khusus untuk audit penetrasi Wi-Fi, manipulasi kerangka kerja IEEE 802.11h CSA, dan analisis simulasi *Rogue AP* (Evil Twin). 

Dibangun ulang secara menyeluruh untuk menggantikan *framework* berbasis Python yang berat, VERITAS menyajikan kecepatan injeksi socket tingkat rendah (*bare-metal*) yang mampu menyuntikkan lebih dari 5.000 paket per detik (PPS) dengan kontrol laju PID presisi sub-milidetik.
...
- [Prasyarat & Persyaratan Sistem](#-prasyarat--persyaratan-sistem)
- [Instalasi & Kompilasi](#-instalasi--kompilasi)
- [Panduan Penggunaan](#-panduan-penggunaan)
  - [1. Mode CLI Interaktif](#1-mode-cli-interaktif)
  - [2. Mode Otomatis / Terjadwal (Konfigurasi JSON)](#2-mode-otomatis--terjadwal-konfigurasi-json)
- [Mekanisme Teknis IEEE 802.11h CSA](#-mekanisme-teknis-ieee-80211h-csa)
- [ Vektor Serangan & Evaluasi Keamanan](#-vektor-serangan--evaluasi-keamanan)
- [📊 Hasil Pengujian & Benchmark Kinerja](#-hasil-pengujian--benchmark-kinerja)
- [🛡️ Strategi Mitigasi & Pengerasan Keamanan (Defensive)](#️-strategi-mitigasi--pengerasan-keamanan-defensive)
- [⚠️ Penyangkalan Etis & Tanggung Jawab Hukum](#️-penyangkalan-etis--tanggung-jawab-hukum)
- [📄 Lisensi](#-lisensi)
...
- **🚀 Injeksi Throughput Ultra-Tinggi**: Menggunakan *raw socket* `AF_PACKET` dengan pengelompokan (*batching*) `sendmmsg()` (hingga 16 paket per *syscall*) serta optimasi *buffer* TX kernel hingga 2MB (`SO_SNDBUF`).
- **🔒 Threading Engine Tanpa Kunci (Lock-Free)**: Eksekusi injeksi multi-vektor bersamaan didukung oleh `<stdatomic.h>` dan deskriptor *socket* per-thread — menghilangkan kontensi kunci (*lock contention*) pada alur eksekusi kritis.
- **🎯 14 Vektor Evaluasi Keamanan Khusus**:
  - `CSA Beacon Flood`: Injeksi IEEE 802.11h Channel Switch Announcement skala besar.
  - `Quiet Element DoS`: Manipulasi elemen periode hening (Quiet IE continuous).
  - `Bidirectional Deauth Flood`: Disosiasi bolak-balik (AP $
ightarrow$ Klien & Klien $
ightarrow$ AP).
  - `Disassociation Flood`: Injeksi paket pemutusan hubungan jaringan secara simultan.
  - `EAPOL Logoff Spoofing`: Konstruksi *raw frame* presisi bit ToDS.
  - `PMKID Auto-Capture`: Filter *handshake* M1 target otomatis & ekspor format Hashcat `22000`.
  - `Auth Table Exhaustion DoS`: Pembuatan identitas palsu masif untuk menghabiskan tabel otentikasi AP.
  - `CSA Action Frame Injection`: Pengiriman *frame* aksi spesifik tanpa mengganggu lalu lintas data utama.
  - `Beacon Confusion / Rogue BSSID`: Penyuntikan BSSID acak untuk mengacaukan pemindaian klien.
  - `Probe Response CSA Spoofing`: Manipulasi *probe response* (Unicast & Broadcast).
  - `DELBA (Delete Block Ack) DoS`: Pemutusan sesi agregasi paket data.
  - `Evil Twin Rogue AP Handoff`: Pengalihan Klien otomatis ke titik akses tiruan.
- **🌐 Pengoperasian Dual-Band (2.4GHz / 5GHz) & DFS Aware**: Dukungan penuh saluran tinggi 5GHz (36–165), konfigurasi hostapd VHT 802.11ac, serta deteksi otomatis saluran DFS (52–64, 100–144).
- **⏱️ Kontrol Laju Transmisi PID (Sliding Window)**: Pengontrol laju transmisi berbasis algoritma PID *rolling window* memastikan kestabilan transmisi sesuai mode agresivitas pilihan (`STEALTH` ~20 PPS hingga `INSANE` ~5000 PPS).
- **🤖 Otomatisasi & Scripting**: Dukungan penuh konfigurasi JSON untuk pengujian otomatis tanpa antarmuka (*headless/red team operations*).
...
```
 ┌─────────────────────────────────────────────────────────────┐
 │                 VERITAS CLI / JSON Engine                   │
 └──────────────────────────────┬──────────────────────────────┘
                                │
 ┌──────────────────────────────▼──────────────────────────────┐
 │             PID Rolling Window Rate Controller              │
 └──────────────────────────────┬──────────────────────────────┘
                                │
 ┌──────────────────────────────▼──────────────────────────────┐
 │          Lock-Free Thread Workers (<stdatomic.h>)           │
 └──────┬───────────────────────┬───────────────────────┬──────┘
        │                       │                       │
 ┌──────▼──────┐         ┌──────▼──────┐         ┌──────▼──────┐
 ...
 └──────┬──────┘         └──────┬──────┘         └──────┬──────┘
        │                       │                       │
 ┌──────┴───────────────────────┴───────────────────────┴──────┐
 │        Linux AF_PACKET Raw Sockets + sendmmsg() Batch       │
 └──────────────────────────────┬──────────────────────────────┘
                                │
 ┌──────────────────────────────▼──────────────────────────────┐
 │                Wireless Network Interface (NIC)             │
 └─────────────────────────────────────────────────────────────┘
```
